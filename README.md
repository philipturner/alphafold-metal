...
>
> If you need ~200 GB freed instead, delete the `pdb_mmcif` dataset. It's slightly more optimized for network bandwidth. I advise to only delete one of (`uniclust30`, `pdf_mmcif`) at a time, unless you need the full 300 GB of disk space. Only free the space you absolutely need, for the sake of your disk's long-term health. For reference, downloading 600 GB on a 1 TB solid-state drive will take off 0.05% of its lifespan. If you have a magnetic hard drive, I strongly recommend using that instead of your SSD.
